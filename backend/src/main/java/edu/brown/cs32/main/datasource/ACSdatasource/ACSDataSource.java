package edu.brown.cs32.main.datasource.ACSdatasource;
...
public class ACSDataSource implements DataSource {
  ...
  public String getCountyCode(String stateCode, String countyName)
    ...
    if (countyConnection.getResponseCode() == 200) {
      ...
      Moshi moshi = new Moshi.Builder().build();
      JsonAdapter<List> adapter = moshi.adapter(List.class).nonNull();
      List<List<String>> jsonData = adapter.fromJson(
          new Buffer().readFrom(countyConnection.getInputStream()));
  ...
  public Map<String, Object> searchACS(String state, String county)
    ...
    // county -> county code
      ...
      String query = "https://api.census.gov/data/2021/acs/acs1/subject/variables?get=NAME,S2802_C03_022E&for=county:" + countyCode + "&in=state:" + stateCode;
    ...
    } catch (InvalidRequestArgumentException | IOException e){throw new InvalidRequestArgumentException(e.getMessage());}
    return null;
  }
