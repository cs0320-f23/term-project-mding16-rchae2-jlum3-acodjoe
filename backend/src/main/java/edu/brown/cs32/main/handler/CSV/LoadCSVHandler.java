...
public class LoadCSVHandler implements Route {
  ...
  public Object handle(Request request, Response response) throws Exception {
    ...
    try {
      ...
      return new ErrorResponse("error_bad_request_invalid_arg_for_" + e.getInvalidField(),
          filePath).serialize();

      // If there is a missing argument in the request, return
      // "error_bad_request_missing_field_for_" + field that is missing
    } catch (MissingArgumentException e) {
      return new ErrorResponse("error_bad_request_missing_field_for_" + e.getMissingField(),
          filePath).serialize();

      // For all other exceptions, return
      // "error_bad_request"
    } catch (Exception e) {
   ...
   * @param hasHeader represents the string input for hasHeader, must be "true" or "false" - case-insensitive
