package edu.brown.cs32.main.handler.CSV;
...
/**
 * Handler for the searchcsv. Searches a loaded csv file for a specified keyword. Can
 * search through all columns or, if a column id or header is provided, returns the relevant search
...
public class SearchCSVHandler implements Route {
  ...
  /** This is the constructor for the SearchCSVHandler class which initializes dataSource
   * and requiredParams
   *
   * @param toUse
   */
  ...
  /**
   * This is the method to search through the loaded csv data by toSearch
   * columnIdentifier, and columnIdentifierValue
   *
   * @param request
   * @param response
   * @return Object
  ...
  public Object handle(Request request, Response response) throws Exception {
    ...
    try{
      ...
      this.searchResults = this.dataSource.search(this.toSearch, this.columnIdentifier, this.columnIdentifierValue);
    ...
    //return new ErrorResponse("error_bad_request: some number format error").serialize();
      ...
      return new ErrorResponse("error_bad_request: data not loaded").serialize();
    } catch (NoHeaderException e){
      return new ErrorResponse("error_bad_request: no header to search from").serialize();
  ...
  public ColumnIdentifier getIdentifierType(String identifier)
    ...
    else {throw new InvalidColumnIdentifierException(
